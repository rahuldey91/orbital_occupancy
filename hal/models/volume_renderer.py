...
class SirenGenerator(nn.Module):
    ...
    def forward(self, x, styles, get_sdf=False):
        if get_sdf:
            return self.get_sdf(x, styles)
    ...
    def get_sdf(self, pts, styles):
        mlp_out = pts.contiguous()
        for i in range(len(self.pts_linears)):
            mlp_out = self.pts_linears[i](mlp_out, styles)
        sdf = self.sigma_linear(mlp_out)
        return sdf
...
class VolumeFeatureRenderer(nn.Module):
    ...
    def volume_integration(self, raw, z_vals, rays_d, pts, return_eikonal=False):
        ...
        return rgb_map, feature_map, sdf_out, mask, xyz, eikonal_term, weights
    ...
    def render_rays(self, ray_batch, styles=None, return_eikonal=False):
        ...
        rgb_map, features, sdf, mask, xyz, eikonal_term, weights = self.volume_integration(raw, z_vals, rays_d, pts, return_eikonal=return_eikonal)

        return rgb_map, features, sdf, mask, xyz, eikonal_term, weights
    ...
    def render(self, focal, c2w, near, far, styles, c2w_staticcam=None, return_eikonal=False):
        ...
        rays = rays.float()
        rgb, features, sdf, mask, xyz, eikonal_term, weights = self.render_rays(rays, styles=styles, return_eikonal=return_eikonal)

        return rgb, features, sdf, mask, xyz, eikonal_term, weights
    ...
    def forward(self, cam_poses, focal, near, far, styles=None, return_eikonal=False):
        rgb, features, sdf, mask, xyz, eikonal_term, weights = self.render(focal, c2w=cam_poses, near=near, far=far, styles=styles, return_eikonal=return_eikonal)
